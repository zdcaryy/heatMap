...
export class LoginService {
    ...
    // }
    login(user: string, pass: string){
      return this.postFormData('http://192.168.31.34:8881',{username:user,password:pass});
    }
    postFormData(key:string,data):Observable<any>{
      ...
      return this.http.post<any>(key,str,httpOptions);
    }
    //字符串转为utf-8
    ...
    // 获取登录用户的信息
    getUserInfo(){
        return this.http.get<any>(`${environment.adminUrl}`+"/user/findByToken", {params:{access_token:localStorage.getItem('access_token')}});
    }
}
